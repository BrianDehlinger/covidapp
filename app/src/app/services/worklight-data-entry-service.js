...
export default class DataEntryService {
  ...
  static async getStats(country) {
    if (!country) {
      return await fetch(apiStatsUrl).then(res => res.json());
    }
    return await fetch(`${apiStatsUrl}?country=${country}`).then(res => res.json());
  }
  ...
  static async setEntriesToIndexedDb(submissionResponse) {
    if (submissionResponse && submissionResponse.data && submissionResponse.data.history) {
      const submissionHistory = submissionResponse.data.history;

      submissionHistory.forEach((item)=>{
        item["timestamp"] = dayjs.utc(item.timestamp).local().format()
      });
