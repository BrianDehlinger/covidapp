...
class LocationResource(Resource):
    ...
    def get(self, country_code=None, **kwargs):
        ...
        if country_code:
            ...
            sql = """
            SELECT
                location_postal_code as post_code_prefix,
                feeling,
                COUNT(*) AS count
            FROM submissions
            WHERE location_country_code = :country_code
            GROUP BY post_code_prefix, feeling
            """
            ...
            for r in result:
                if r.post_code_prefix not in data['submissions']:
                    data['submissions'][r.post_code_prefix] = []

                data['submissions'][r.post_code_prefix].append({
                    'count': r.count,
                    'feeling': r.feeling
                })
