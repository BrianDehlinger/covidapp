...
test-extra:
	...
	yamllint -d "{extends: default, rules: {line-length: {level: warning}}}" $(shell find . \( -name "*.yml" -or -name "*.yaml" \) -not -path "*/node_modules/*" | xargs)
...
update-review:
	# Ensure latest commit on named branch is checked out. 
        # This command is run in the staging environment.
